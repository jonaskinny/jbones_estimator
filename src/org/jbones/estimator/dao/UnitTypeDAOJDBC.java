...
*/
   public class UnitTypeDAOJDBC extends ADAOJDBC {
   protected String getEntityName() {
      return "Unit_Type";
   }
   protected String getArchiveEntityName() {
      return "Unit_Type_Archive";
   }
   /**
   ...
   */
   public boolean create(DTO arg) throws CreateException, UniqueException {
      ...
      int result = -1;
      try {
         UnitType unitType = (UnitType)arg;
         conn = getConnection();
         // transaction begin
         JDBCUtil.setAutoCommit(conn,false);
         ps = conn.prepareStatement("insert into " + getSchemaName() + "." + "ENTITY_UNIT_TYPE A values(?)");
         entityId = getSequenceNextVal();
         ps.setLong(1,entityId); // used for PK in entity table
         result = ps.executeUpdate();
         if (result == 1) {
            ps = conn.prepareStatement("insert into " + getSchemaName() + "." + "UNIT_TYPE A values(?,?)");
            ps.setLong(1,entityId);
            ps.setString(2,unitType.getIdentifier());
            result = ps.executeUpdate();
         }
         return result == 1;
   ...
   */
   private boolean createArchive(DTO arg, Connection conn) throws CreateException, UniqueException {
      ...
      try {
         UnitType unitType = (UnitType)arg;
         ps = conn.prepareStatement("insert into " + getSchemaName() + "." + "UNIT_TYPE_ARCHIVE A values(?,?,?)");
         ps.setLong(1,getArchiveSequenceNextVal());
         ps.setLong(2,unitType.getId());
         ps.setString(3,unitType.getIdentifier());
         result = ps.executeUpdate();
         return result == 1;
         // improve exception throwing here
      } catch (SQLIntegrityConstraintViolationException sqlICVE) {
   ...
   */
	public DTO read(long arg, Connection conn) throws ReadException {
      ...
      ResultSet rs = null;
      try {
         ps = conn.prepareStatement("select A.UNIT_TYPE_ID, A.IDENTIFIER from " + getSchemaName() + "." + "UNIT_TYPE A where A.UNIT_TYPE_ID = ? order by A.UNIT_TYPE_ID ASC");
         ps.setLong(1,arg);
         rs = ps.executeQuery();
         if (rs.next()) {
            return loadEntity(new UnitType(), conn, rs);
         }
         return null;
         // improve exception throwing here
      } catch (SQLException sqlE) {
   ...
   public DTO readList(long arg, Connection conn) throws ReadException {
      ...
      try {
         ...
         rs = ps.executeQuery();
         if (rs.next()) {
            return loadIdentifierEntity(new UnitType(), conn, rs);
         }
         return null;
         // improve exception throwing here
      } catch (SQLException sqlE) {
   ...
   */
	public DTO readArchive(long arg, Connection conn) throws ReadException {
      ...
      ResultSet rs = null;
      try {
         ps = conn.prepareStatement("select A.ARCHIVE_ID, A.UNIT_TYPE_ID, A.IDENTIFIER from " + getSchemaName() + "." + "UNIT_TYPE_ARCHIVE A where A.ARCHIVE_ID = ?");
         ps.setLong(1,arg);
         rs = ps.executeQuery();
         if (rs.next()) {
            return loadEntityArchive(new UnitType(), conn, rs);
         }
         return null;
         // improve exception throwing here
      } catch (SQLException sqlE) {
   ...
   */
	public List list(Connection conn) throws ListException {
      ...
      ResultSet rs = null;
      try {
         ps = conn.prepareStatement("select A.UNIT_TYPE_ID, A.IDENTIFIER from " + getSchemaName() + "." + "UNIT_TYPE A order by A.UNIT_TYPE_ID ASC");
         rs = ps.executeQuery();
         List list = new ArrayList<UnitType>();
         while (rs.next()) {
            list.add(loadListEntity(new UnitType(), conn, rs));
         }
         return list;
         // improve exception throwing here
      } catch (SQLException sqlE) {
   ...
   */
	public List listArchive(Connection conn) throws ListException {
      ...
      ResultSet rs = null;
      try {
         ps = conn.prepareStatement("select A.ARCHIVE_ID, A.UNIT_TYPE_ID, A.IDENTIFIER from " + getSchemaName() + "." + "UNIT_TYPE_ARCHIVE A order by A.ARCHIVE_ID ASC");
         rs = ps.executeQuery();
         List list = new ArrayList<UnitType>();
         while (rs.next()) {
            list.add(loadEntityArchive(new UnitType(), conn, rs));
         }
         return list;
         // improve exception throwing here
      } catch (SQLException sqlE) {
   ...
   */
	public boolean update(DTO arg) throws UpdateException, UniqueException {
      ...
      try {
         UnitType unitType = (UnitType)arg;
         conn = getConnection();
         ps = conn.prepareStatement("update " + getSchemaName() + "." + "UNIT_TYPE A set A.IDENTIFIER = ? where A.UNIT_TYPE_ID = ?");
         ps.setString(1,unitType.getIdentifier());
         ps.setLong(2,unitType.getId());
         result = ps.executeUpdate();
         return result == 1;
         // improve exception throwing here
   ...
   */
	public boolean archive(long arg) throws ArchiveException, UniqueException {
      ...
      Connection conn = null;
      try {
         unitType = (UnitType) read(arg); // read based on the entity id
         if (null == unitType) {
            return false;
         }
         // transaction begins
         ...
         if (createArchive(unitType, conn)) {
            if (delete(arg,conn)) {
               JDBCUtil.commit(conn);
               return true;
            } else {
   ...
   */
	public boolean unarchive(long arg) throws CreateException, UniqueException {
      ...
      Connection conn = null;
      try {
         unitType = (UnitType) readArchive(arg); // read based on the archive id
         if (null == unitType) {
            return false;
         }
         // transaction begins
         conn = getConnection();
         conn.setAutoCommit(false);
         if (createFromArchive(unitType, conn)) {
   ...
   */
	private boolean delete(long arg, Connection conn) throws DeleteException {
      ...
      int result = -1;
      try {
         ps = conn.prepareStatement("delete from " + getSchemaName() + "." + "UNIT_TYPE A where A.UNIT_TYPE_ID = ?");
         ps.setLong(1,arg);
         result = ps.executeUpdate();
         return result == 1;
         // improve exception throwing here
   ...
   */
	private boolean deleteArchive(long arg, Connection conn) throws DeleteException {
      ...
      int result = -1;
      try {
         ps = conn.prepareStatement("delete from " + getSchemaName() + "." + "UNIT_TYPE_ARCHIVE A where A.ARCHIVE_ID = ?");
         ps.setLong(1,arg);
         result = ps.executeUpdate();
         return result == 1;
         // improve exception throwing here
   ...
   */
   ...
   protected DTO loadEntity(DTO entity, Connection conn, ResultSet rs) throws ReadException {
      ...
      try {
         unitType.setId(rs.getLong(1));
         unitType.setIdentifier(rs.getString(2));
         return unitType;
      }  catch (SQLException sqlE) {
            throw new ReadException(sqlE.getMessage());
      }
   ...
   protected DTO loadEntityArchive(ADTO entity, Connection conn, ResultSet rs) throws ReadException {
      UnitType unitType = (UnitType)entity;
      try {
         unitType.setArchiveId(rs.getLong(1));
         unitType.setId(rs.getLong(2));
         unitType.setIdentifier(rs.getString(3));
         return unitType;
      }  catch (SQLException sqlE) {
            throw new ReadException(sqlE.getMessage());
      }
