...
*/
   public class PhaseDAOJDBC extends ADAOJDBC {
   ...
   */
   public boolean create(DTO arg) throws CreateException, UniqueException {
      ...
      try {
         ...
         ps = conn.prepareStatement("insert into " + getSchemaName() + "." + "ENTITY_PHASE A values(?)");
         entityId = getSequenceNextVal();
         ps.setLong(1,entityId); // used for PK in entity table
         result = ps.executeUpdate();
         if (result == 1) {
            ps = conn.prepareStatement("insert into " + getSchemaName() + "." + "PHASE A values(?,?)");
            ps.setLong(1,entityId);
            ps.setString(2,phase.getIdentifier());
            result = ps.executeUpdate();
         }
         return result == 1;
   ...
   */
   private boolean createArchive(DTO arg, Connection conn) throws CreateException, UniqueException {
      ...
      try {
         Phase phase = (Phase)arg;
         ps = conn.prepareStatement("insert into " + getSchemaName() + "." + "PHASE_ARCHIVE A values(?,?,?)");
         ps.setLong(1,getArchiveSequenceNextVal());
         ps.setLong(2,phase.getId());
         ps.setString(3,phase.getIdentifier());
         result = ps.executeUpdate();
         return result == 1;
         // improve exception throwing here
      } catch (SQLIntegrityConstraintViolationException sqlICVE) {
   ...
   */
	public DTO read(long arg, Connection conn) throws ReadException {
      ...
      ResultSet rs = null;
      try {
         ps = conn.prepareStatement("select A.PHASE_ID, A.IDENTIFIER from " + getSchemaName() + "." + "PHASE A where A.PHASE_ID = ? order by A.PHASE_ID ASC");
         ps.setLong(1,arg);
         rs = ps.executeQuery();
         if (rs.next()) {
            return loadEntity(new Phase(), conn, rs);
         }
         return null;
         // improve exception throwing here
      } catch (SQLException sqlE) {
   ...
   public DTO readList(long arg, Connection conn) throws ReadException {
      ...
      try {
         ...
         rs = ps.executeQuery();
         if (rs.next()) {
            return loadListEntity(new Phase(), conn, rs);
         }
         return null;
         // improve exception throwing here
      } catch (SQLException sqlE) {
   ...
   */
	public DTO readArchive(long arg, Connection conn) throws ReadException {
      ...
      ResultSet rs = null;
      try {
         ps = conn.prepareStatement("select A.ARCHIVE_ID, A.PHASE_ID, A.IDENTIFIER from " + getSchemaName() + "." + "PHASE_ARCHIVE A where A.ARCHIVE_ID = ?");
         ps.setLong(1,arg);
         rs = ps.executeQuery();
         if (rs.next()) {
            return loadEntityArchive(new Phase(), conn, rs);
         }
         return null;
         // improve exception throwing here
      } catch (SQLException sqlE) {
   ...
   */
	public List list(Connection conn) throws ListException {
      ...
      ResultSet rs = null;
      try {
         ps = conn.prepareStatement("select A.PHASE_ID, A.IDENTIFIER from " + getSchemaName() + "." + "PHASE A order by A.PHASE_ID ASC");
         rs = ps.executeQuery();
         List list = new ArrayList<Phase>();
         while (rs.next()) {
            list.add(loadListEntity(new Phase(), conn, rs));
         }
         return list;
         // improve exception throwing here
      } catch (SQLException sqlE) {
   ...
   */
	public List listArchive(Connection conn) throws ListException {
      ...
      ResultSet rs = null;
      try {
         ps = conn.prepareStatement("select A.ARCHIVE_ID, A.PHASE_ID, A.IDENTIFIER from " + getSchemaName() + "." + "PHASE_ARCHIVE A order by A.ARCHIVE_ID ASC");
         rs = ps.executeQuery();
         List list = new ArrayList<Phase>();
         while (rs.next()) {
            list.add(loadEntityArchive(new Phase(), conn, rs));
         }
         return list;
         // improve exception throwing here
      } catch (SQLException sqlE) {
   ...
   */
	public boolean update(DTO arg) throws UpdateException, UniqueException {
      ...
      try {
         Phase phase = (Phase)arg;
         conn = getConnection();
         ps = conn.prepareStatement("update " + getSchemaName() + "." + "PHASE A set A.IDENTIFIER = ? where A.PHASE_ID = ?");
         ps.setString(1,phase.getIdentifier());
         ps.setLong(2,phase.getId());
         result = ps.executeUpdate();
         return result == 1;
         // improve exception throwing here
   ...
   */
	public boolean archive(long arg) throws ArchiveException, UniqueException {
      ...
      Connection conn = null;
      try {
         phase = (Phase) read(arg); // read based on the entity id
         if (null == phase) {
            return false;
         }
         // transaction begins
   ...
   */
	public boolean unarchive(long arg) throws CreateException, UniqueException {
      ...
      Connection conn = null;
      try {
         phase = (Phase) readArchive(arg); // read based on the archive id
         if (null == phase) {
            return false;
         }
         // transaction begins
         conn = getConnection();
         conn.setAutoCommit(false);
         if (createFromArchive(phase, conn)) {
   ...
   */
	private boolean delete(long arg, Connection conn) throws DeleteException {
      ...
      int result = -1;
      try {
         ps = conn.prepareStatement("delete from " + getSchemaName() + "." + "PHASE A where A.PHASE_ID = ?");
         ps.setLong(1,arg);
         result = ps.executeUpdate();
         return result == 1;
         // improve exception throwing here
   ...
   */
	private boolean deleteArchive(long arg, Connection conn) throws DeleteException {
      ...
      int result = -1;
      try {
         ps = conn.prepareStatement("delete from " + getSchemaName() + "." + "PHASE_ARCHIVE A where A.ARCHIVE_ID = ?");
         ps.setLong(1,arg);
         result = ps.executeUpdate();
         return result == 1;
         // improve exception throwing here
   ...
   */
   ...
   protected DTO loadEntity(DTO entity, Connection conn, ResultSet rs) throws ReadException {
      ...
      try {
         phase.setId(rs.getLong(1));
         phase.setIdentifier(rs.getString(2));
         return phase;
      }  catch (SQLException sqlE) {
            throw new ReadException(sqlE.getMessage());
      }
   ...
   protected DTO loadEntityArchive(ADTO entity, Connection conn, ResultSet rs) throws ReadException {
      Phase phase = (Phase)entity;
      try {
         phase.setArchiveId(rs.getLong(1));
         phase.setId(rs.getLong(2));
         phase.setIdentifier(rs.getString(3));
         return phase;
      }  catch (SQLException sqlE) {
            throw new ReadException(sqlE.getMessage());
      }
