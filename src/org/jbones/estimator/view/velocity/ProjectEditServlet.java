...
public class ProjectEditServlet extends VelocityViewServlet {
   ...
   public Template handleRequest(HttpServletRequest request,HttpServletResponse response, Context context) {
      try {
         ...
         // jcole fix npe here where user manually enters update url but no ProjectActionBean is in request yet.
         context.put("project", projectActionBean.getProject());
         context.put("projectTypeList", projectActionBean.getProjectTypeList());
         context.put("projectMessageList", MessageConverter.convert(projectActionBean.getContext().getMessages()));

         Template t = null;
         
         t = getTemplate("ProjectEdit.vm");
